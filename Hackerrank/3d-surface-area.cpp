...
int surfaceArea(vector<vector<int>> A) {
    ...
    for(int i = 0; i < A.size(); i++){
        for(int j = 0; j < A[i].size(); j++){
            price += 2;
            price += (i > 0 ? max(0, A[i][j] - A[i - 1][j]) : A[i][j]);
            price += (j > 0 ? max(0, A[i][j] - A[i][j - 1]) : A[i][j]);
            price += (i + 1 < A.size() ? max(0, A[i][j] - A[i + 1][j]) : A[i][j]);
            price += (j + 1 < A[i].size() ? max(0, A[i][j] - A[i][j + 1]) : A[i][j]);        
        }
    }
